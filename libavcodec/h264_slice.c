...
static int init_table_pools(H264Context *h)
{
    ...
    h->sub_mb_type_pool  = av_buffer_pool_init((big_mb_num + h->mb_stride) *
                                               sizeof(uint16_t[4]), av_buffer_allocz);
...
static int alloc_picture(H264Context *h, H264Picture *pic)
{
    ...
    pic->sub_mb_type_buf  = av_buffer_pool_get(h->sub_mb_type_pool);
    if (!pic->qscale_table_buf || !pic->mb_type_buf || !pic->sub_mb_type_buf)
        goto fail;

    pic->mb_type      = (uint32_t*)pic->mb_type_buf->data + 2 * h->mb_stride + 1;
    pic->sub_mb_type  = (uint16_t (*)[4])pic->sub_mb_type_buf->data + 2 * h->mb_stride + 1;
